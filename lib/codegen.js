...
const genApiCode = (data) => {
    ...
    const template = config.applyPluginsWaterfall('api-template', apiTmpl, data)
...
const genServiceCode = (data) => {
    ...
    let template = config.applyPluginsWaterfall('service-template', serviceTmpl, data)
    genSourceCode(data, template, serviceFile) // service source

    // @plugin api-template 测试文件模板
    template = config.applyPluginsWaterfall('test-template', testTmpl, data)
