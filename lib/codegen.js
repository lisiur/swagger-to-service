...
let serviceDTSFile = null
let serviceTestFile = null
let yamlDTSFile = null
...
const prepareDir = () => {
  ...
  serviceDTSFile = confs.serviceDTSPath
  yamlDTSFile = confs.yamlDTSPath
  serviceTestFile = confs.utPath

  mkdirp(path.dirname(apiFile))
  mkdirp(path.dirname(serviceFile))
  mkdirp(path.dirname(serviceDTSFile))
  mkdirp(path.dirname(yamlDTSFile))
...
const SERVICE_DTS_TMPL = path.resolve(__dirname, 'tmpl', 'serviceDTS.js.tmpl')
...
const genApiCode = data => {
  ...
  const template = Handlebars.compile(
    config.applyPluginsWaterfall(
      'api-template',
      fs.readFileSync(API_TMPL, 'utf-8'),
      data
    )
  )
...
const genServiceCode = data => {
  ...
  let template = Handlebars.compile(
    config.applyPluginsWaterfall(
      'service-template',
      fs.readFileSync(SERVICE_TMPL, 'utf-8'),
      data
    )
  )
  ...
  template = Handlebars.compile(
    config.applyPluginsWaterfall(
      'test-template',
      fs.readFileSync(TEST_TMPL, 'utf-8'),
      data
    )
  )
...
const genBeansDefinitions = async (yamlPath, targetFile) => {
  const content = await SwaggerParser.parse(yamlPath)
  const fileContent = await dtsGenerator({
    contents: [content]
  })
  try {
    fs.writeFileSync(targetFile, fileContent, 'utf-8')
...
// 生成 service.d.ts
const genServiceDTSCode = async data => {
  await genBeansDefinitions(config.all().url, yamlDTSFile)
  if (!data) {
    console.error('generate code failed !')
    return
  }
  ...
  let template = Handlebars.compile(
    config.applyPluginsWaterfall(
      'serviceDTS-template',
      fs.readFileSync(SERVICE_DTS_TMPL, 'utf-8'),
      data
    )
  )
  genSourceCode(data, template, serviceDTSFile) // service source
}
